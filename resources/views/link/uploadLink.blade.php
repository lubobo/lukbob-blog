...
@section('content')
    <div class="panel panel-default col-xs-6 col-xs-offset-3">
        ...
        <div class="panel panel-body">
            <form class="form-group col-xs-5 col-xs-offset-4" action="{{route('postUploadLink')}}" method="POST">
                <span class="glyphicon glyphicon-transfer"> 友链IP:</span>
                <br>
                <input class="form-control" name="ip" onmouseout="check_ip(this.value)" placeholder="请输入友链IP..." value="{!! $link->ip !!}">
                <h5><span class="text-danger" id="ip"></span></h5>
                <br>
                <span class="glyphicon glyphicon-globe"> 友链name:</span>
                <br>
                <input class="form-control" name="name" onmouseout="check_name(this.value)" placeholder="请输入友链name..." value="{!! $link->name !!}">
                <h5><span class="text-danger" id="name"></span></h5>
                <br>
                <span class="glyphicon glyphicon-user"> 友链owner:</span>
                <br>
                <input class="form-control" name="owner" placeholder="请输入友链owner..." value="{!! $link->owner !!}">
                <br>
                <span class="glyphicon glyphicon-cloud"> 友链status:</span>
                <br>
                <textarea class="form-control" rows="10" name="status" placeholder="请输入友链status...">{!! $link->status !!}</textarea>
